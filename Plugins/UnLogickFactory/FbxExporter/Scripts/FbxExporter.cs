	...
	public class FbxExporter
	{
		...
		public static bool Export(string filename, FbxExportSettings settings, params Transform[] roots)
		{
			...
			try
			{
				...
				try
				{
					...
					foreach (var root in roots)
					{
                        ...
                        if ((settings.objectExportMask & ObjectExportMask.ExportSkinnedMeshes) == ObjectExportMask.ExportSkinnedMeshes && settings.skinnedMeshOptions == SkinnedMeshOptions.ExportTPose)
                        {
                            ...
                            if (WriteDefaultPoseMethod != null)
                            {
                                ...
                                {
                                    WriteDefaultPoseMethod.Invoke(animator, null);
                                }
                            }
                            else
                            {
                                LogErrorFormat(settings.logLevel >= 0, "FbxExporter - Failed to set TPose, it seems unity {0} doesn't have the Animator.WriteDefaultPose method.", Application.unityVersion);
                            }
                        }
					...
					var success = UnityFbxExporterBinding.Save(outputFilename, settings.formatId);
					System.IO.Directory.SetCurrentDirectory(previousDir);
					if (success)
					{
						LogFormat(settings.logLevel >= 0, "FbxExporter - Fbx file generated {0}", outputFilename);
					}
					else
					{
						LogErrorFormat(settings.logLevel >= 0, "FbxExporter - Failed to write {0}", outputFilename);
					}
#if UNITY_EDITOR
                    FixImportSettings(filename);
#endif
                    return success;
				}
				finally
		...
		private static void ExportSkinnedMeshOrCloth(SkinnedMeshRenderer smr, Transform root, TextureExporter textureExporter, string outputPath, Dictionary<IntPtr, IntPtr> skeletonNodes, Dictionary<Material, IntPtr> materialDictionary, FbxExportSettings settings, FbxExportCollection fbxCollection)
		{
			...
			if (string.IsNullOrEmpty(mesh.name))
				mesh.name = smr.name;
			var fbxMesh = UnityFbxExporterBinding.ProcessMesh(mesh, settings.logLevel, materials, vertices, normals, node, materialDictionary, out fbxMaterials, out fbxTextures);
...
#if UNITY_EDITOR
		public static void FixImportSettings(string path)
		{
		...
		public static void FixImportSettings(GameObject fbx)
		{
			...
			if (!string.IsNullOrEmpty(assetPath))
			{
				...
				var hasSkinnedMesh = smrs != null && smrs.Length > 0;

				if (mi.globalScale != 100f || (hasSkinnedMesh && mi.animationType == ModelImporterAnimationType.None) || (!hasSkinnedMesh && mi.animationType != ModelImporterAnimationType.None))
				{
					mi.globalScale = 100f;
					if (hasSkinnedMesh && mi.animationType == ModelImporterAnimationType.None)
		...
		public static string Version { get { return "1.4.1"; } }
