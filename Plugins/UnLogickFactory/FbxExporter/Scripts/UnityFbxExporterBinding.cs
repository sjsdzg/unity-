...
namespace UnLogickFactory
{
	public static class UnityFbxExporterBinding
	{
        ...
        const string dllVersion = "14";
        const string dllExtension = "dll";

        public static string GetDLLPath(int logLevel = 0)
        {
            var dllFilename = string.Format("{0}_{1}_v{2}.{3}", dllFilenameBase, IntPtr.Size == 8 ? "x86_64" : "x86", dllVersion, dllExtension);
		...
		public static BoolStringInt Save;
			...
			if (blendShapeCount > 0)
			{
				...
				for (int i = 0; i < blendShapeCount; i++)
				{
					...
					for (int j = 0; j < frames; j++)
					{
						...
						Debug.LogFormat("BlendShape: {0} Weight: {1}", blendShapeName, weight);
