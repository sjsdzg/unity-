...
{
    class Panel_SelectInformation : MonoBehaviour
    {
        ...
        public void InitListViewIcons()
        {
            ...
            //给节点赋值
            item.Name = DateTime.Now.ToString("yyyyMMdd") + "01";

            //在栈上增加一个新节点---m_List
            ObservableList<ListViewIconsItemDescription> m_List = new ObservableList<ListViewIconsItemDescription>();
            m_List.Add(item);

            //读取数据资源
            listBatch.DataSource = m_List;

            text_Batch.text = item.Name;
        }
