...
{
    public class MyScorePanel : AbstractPanel
    {
        public const string COLUMN_NUMBER = "Number";//序号列
        public const string COLUMN_EXAMNAME = "ExamName";//考试名称列
        public const string COLUMN_SCORE = "Score";//考试分数
        public const string COLUMN_PAPERSCORE = "PaperScore";//试卷分数列
        public const string COLUMN_DURATION = "Duration";//考试耗时列
        public const string COLUMN_ENDTime = "EndTime";//交卷时间列

        public override EnumPanelType GetPanelType()
        {
            return EnumPanelType.MyScorePanel;
        }
        ...
        /// 考试查询栏目
        /// </summary>
        private MyScoreQueryBar queryBar;
        ...
        private void InitGUI()
        {
            ...
            queryBar = transform.Find("QueryBar").GetComponent<MyScoreQueryBar>();
        }
        ...
        protected override void OnStart()
        {
            base.OnStart();
            //赋值用户名
            queryBar.UserName = GlobalManager.user.UserName;
        ...
        protected override void SetPanel(params object[] PanelParams)
        {
            ...
            addressBar.AddHyperButton(EnumPanelType.ManualGradingPanel, PanelDefine.GetPanelComment(EnumPanelType.MyScorePanel));
        }
        ...
        private void pageDataGrid_ButtonCellClick(DataGridViewRow row, ButtonCellType type)
        {
            ...
            switch (type)
            {
                case ButtonCellType.Detail:
                    ...
                    UserPaperPanelData data = new UserPaperPanelData();
                    data.ExamDataInfo = examData;

                    if (string.IsNullOrEmpty(data.ExamDataInfo.PaperId))
                    {
                        MessageBoxEx.Show("<color=red>经过系统检测，考试试卷被删除，无法评卷。</color>", "提示", MessageBoxExEnum.SingleDialog, null);
                    }
                    else
                    {
                        PanelManager.Instance.OpenPanel(popupPanelContainer, EnumPanelType.UserPaperPanel, data);
                    }
                    break;
                default:
                    break;
        ...
        private void ReceiveListExamCategoryByConditionResp(NetworkPackageInfo packageInfo)
        {
            ...
            //获取完题库之后, 分页查询考试。
            //examDataModule.PageExamData(1, pageDataGrid.PageSize, ReceivePageExamDataResq);
            examDataModule.PageExamDataByCondition(pageDataGrid.CurrentPage, pageDataGrid.PageSize, queryBar.SqlConditions, ReceivePageExamDataByCondition);
        }
        ...
        private ObservableList<DataGridViewRowData> BuildDataSource(RepeatedField<ExamDataProto> examDataProtos)
        {
            ...
            for (int i = 0; i < examDataProtos.Count; i++)
            {
                ...
                //考试名称
                rowData.CellValueDict.Add(COLUMN_EXAMNAME, examData.ExamName);
                //分数
                rowData.CellValueDict.Add(COLUMN_SCORE, examData.Score);
                //分数
                rowData.CellValueDict.Add(COLUMN_PAPERSCORE, examData.PassScore + " / " + examData.TotalScore);
                //考试耗时
                rowData.CellValueDict.Add(COLUMN_DURATION, (examData.EndTime - examData.StartTime).Minutes + "分钟");
                //交卷时间
