...
namespace XFramework.UI
{
    public class UserView : MonoBehaviour
    {
        ...
        /// 试题组件列表
        /// </summary>
        private List<UserViewItem> UserViewItems = null;
        ...
        {
            UserViewItems = new List<UserViewItem>();
            if (DefaultItem == null)
            {
        ...
        public void AddQuestion(Question question)
        {
            ...
            UserViewItem component = obj.GetComponent<UserViewItem>();

            if (Content != null && component != null)
            {
                component.transform.SetParent(Content, false);
                obj.layer = Content.gameObject.layer;
                UserViewItems.Add(component);
                component.OnClicked.AddListener(component_OnClicked);
                component.Number = StartNum + UserViewItems.Count;
            }
        }

        public void TurnOn(int number, bool value)
        {
            if (number > StartNum && number <= UserViewItems.Count + StartNum)
            {
                UserViewItem examViewItem = UserViewItems[number - StartNum - 1];
        ...
        public void Clear()
        {
            for (int i = 0; i < UserViewItems.Count; i++)
            {
                UserViewItem component = UserViewItems[i];
