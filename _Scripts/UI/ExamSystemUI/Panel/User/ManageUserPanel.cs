...
namespace XFramework.UI
{
    ...
    /// 管理用户Panel
    /// </summary>
    public class ManageUserPanel : AbstractPanel
    {
        ...
        {
            return EnumPanelType.ManageUserPanel;
        }
        ...
        /// 用户查询栏目
        /// </summary>
        private UserQueryBar userQueryBar;
        ...
        private void InitGUI()
        {
            ...
            userQueryBar = transform.Find("QueryBar").GetComponent<UserQueryBar>();
        }
        ...
        private void InitEvent()
        {
            ...
            userQueryBar.OnQuery.AddListener(userQueryBar_OnQuery);
        }
        ...
        protected override void SetPanel(params object[] PanelParams)
        {
            ...
            addressBar.AddHyperButton(EnumPanelType.ManageUserPanel, PanelDefine.GetPanelComment(EnumPanelType.ManageUserPanel));
        }
        ...
        {
            userModule.PageUserByCondition(currentPage, pageSize, userQueryBar.SqlConditions, ReceivePageUserByCondition);
        }
        ...
        private void userQueryBar_OnQuery()
        {
            ...
            userModule.PageUserByCondition(pageDataGrid.CurrentPage, pageDataGrid.PageSize, userQueryBar.SqlConditions, ReceivePageUserByCondition);
        }
        ...
        private void pageDataGrid_ButtonCellClick(DataGridViewRow row, ButtonCellType type)
        {
            ...
            switch (type)
            {
                case ButtonCellType.Update:
                    PanelManager.Instance.OpenPanelCloseOthers(this.Parent, EnumPanelType.AddOrModifyUserPanel, OperationType.Modify, user);
                    break;
                case ButtonCellType.Delete:
                    if (user.Id.Equals(GlobalManager.user.Id))
                    {
                        MessageBoxEx.Show("<color=red>不能对其[" + GlobalManager.user.UserName + "]进行删除操作。</color>", "提示", MessageBoxExEnum.SingleDialog, null);
                    }
                    else
                    {
                        MessageBoxEx.Show("<color=red>您确定要删除吗？</color>", "提示", MessageBoxExEnum.CommonDialog, x =>
                        {
                            ...
                            if (flag)
                            {
                                userModule.DeleteUser(user.Id, ReceiveDeleteUserResp);
                            }
                        });
                    }
                    break;
        ...
        private void batchActionBar_ButtonCellClick(BatchActionBar arg0, ButtonCellType type)
        {
            switch (type)
            {
                case ButtonCellType.BatchDelete:
                    ...
                    //判断
                    if (list.Contains(GlobalManager.user.Id))
                    {
                        MessageBoxEx.Show("<color=red>不能对其[" + GlobalManager.user.UserName + "]进行删除操作。</color>", "提示", MessageBoxExEnum.SingleDialog, null);
                    }
                    else
        ...
        private void ReceiveListAllBranchResp(NetworkPackageInfo packageInfo)
        {
            ...
            //设置用户库下拉框
            userQueryBar.SetBranchs(branchs);
            m_Branchs = branchs;
            //获取完专业之后, 分页查询用户。
        ...
        private void ReceiveListAllPositionResp(NetworkPackageInfo packageInfo)
        {
            ...
            //设置用户库下拉框
            userQueryBar.SetPositions(positions);
