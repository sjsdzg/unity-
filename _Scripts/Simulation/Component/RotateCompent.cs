...
namespace XFramework.Component
{
    ...
    /// 旋转组件（一直旋转或停）
    /// </summary>
    public class RotateCompent : ComponentBase
    {
        /// <summary>
        /// 轴向
        /// </summary>
        ...
        void Update()
        {
            if (IsOn)
            {
                ...
                {
                    case RotationAxis.X:                        
                        gameObject.transform.Rotate(Vector3.right, Time.deltaTime* torque);                        
                        break;
                    case RotationAxis.Y:
                        gameObject.transform.Rotate(Vector3.up, Time.deltaTime* torque);
                        break;
                    case RotationAxis.Z:
                        gameObject.transform.Rotate(Vector3.forward, Time.deltaTime* torque);
                        break;
                    default:
                        break;
                }
