...
public class PLC_200L : MonoBehaviour
{
    /// <summary>
    /// PH值
    /// </summary>
    ...
    private void Awake()
    {
        EventDispatcher.RegisterEvent<string>("200L培养罐阀门点击前", ValveBeforeEvent);
        EventDispatcher.RegisterEvent<string>("200L培养罐液位参数变化", liquidTextChange_callBack);
    ...
    private void SetAddSumInputFieldEvent(IOSetPanelType arg1)
    {
        if (arg1.Equals(iOSetPanel_3L.type))
        {
            ...
            {              
                if (addSumInputField.text.Equals("150"))
                {
    ...
    private void Start()
    {
        ...
        temText = transform.Find("backGround/parameterSetBar/speedText").GetComponent<Text>();
        speedText = transform.Find("backGround/parameterSetBar/temText").GetComponent<Text>();
    ...
    private void ValveAfterEvent(string name)
    {
        ...
        if (name.Equals("3-2-2") || name.Equals("3-2-3"))
        {
            EventDispatcher.ExecuteEvent<string>("2D阀门自动关闭", name);
            if (name.Equals("3-2-3"))
            {
                EventDispatcher.ExecuteEvent<string>("200L培养罐进料结束", "3-2-3-2");
            }
            return;
        }
        ...
        if (name.Equals("3-2-1-1"))
        {
            EventDispatcher.ExecuteEvent("200L罐加入培养基");
        }
    }
    private void Hide()
    {
    ...
    {
        EventDispatcher.UnregisterEvent<string>("200L培养罐阀门点击前", ValveBeforeEvent);
        EventDispatcher.UnregisterEvent<string>("200L培养罐液位参数变化", liquidTextChange_callBack);
