...
namespace Battlehub.RTHandles
{
    ...
    public class RuntimeSelectionComponent : RTEComponent, IRuntimeSelectionComponent
    {
        ...
        {
            UseColliders, 
            UseRenderers, //issues with static objects + impossible to select terrain
            UseRenderersBeforeColliders,
        }

        [SerializeField]
        private SelectionMode m_selectionMode = SelectionMode.UseColliders;
        ...
        protected override void Awake()
        {
            ...
            m_selectionPicker = new SelectionPicker(Window, args => OnSelectionFiltering(this, args));
        ...
        {
            return UnityObjectExt.FindObjectsByType<BaseRaycaster>().Where(raycaster => raycaster.GetComponentInParent<IRTE>() == null);
        }
        ...
        protected virtual IList<RaycastResult> RaycastUIObjects()
        {
            ...
            foreach (BaseRaycaster raycaster in FindRaycasters())
            {
                ...
                if (raycaster is GraphicRaycaster)
                {
                    ...
                    //GraphicsRaycaster.m_BlockingMask is not accessible. Make sure to exclude Layers 16 - 24 from m_BlockingMask manually.
                    //see Battlehub/RTEditor/Content/Runtime/RTEditor/BuiltinMenu/GameObjects/UI/Canvas.prefab/GraphicRaycaster for details.
        ...
        {
            RaycastHit[] hits = Physics.RaycastAll(Window.Pointer, float.MaxValue);
        ...
        {
            Renderer[] renderers = m_selectionPicker.Pick();
            return renderers.Where(r => CanSelectObject(r.gameObject)).OrderBy(hit => GetDepth(hit.transform)).ToArray();
        }
        ...
        public virtual void SelectGO(bool multiselect, bool allowUnselect)
        {
            ...
            {
                ProcessUIObjects(multiselect, allowUnselect, raycastResults);
            }
            else
            {
                if (m_selectionMode == SelectionMode.UseRenderers)
                {
        ...
        private bool TrySelectRenderer(bool multiselect, bool allowUnselect)
        {
            ...
            GameObject hitGO = GetNextGo(renderers, r => r.gameObject);
        ...
        private void UpdateSelection(bool multiselect, UnityObject[] selection)
        {
            ...
            else
            {
                if (multiselect)
                {
                    ...
                    {
                        Editor.Undo.Select(Selection, selection, selection.FirstOrDefault());
                    }
                    else
                    {
        ...
        private int GetNextIndex<T>(IList<T> hits, Func<T, GameObject> getGameObject)
        {
            ...
            Ray pointer = Window.Pointer;
            if(m_prevPointer.origin == pointer.origin && m_prevPointer.direction == pointer.direction)
            {
        ...
        {
            if (Editor.Tools.SelectionMode == RTCommon.SelectionMode.Root)
            {
        ...
        public void BoxSelect(GameObject[] gameObjects, bool multiselect)
        {
            ...
            IEnumerable<GameObject> uiObjects = BoxcastUIObjects();
            UnityObject[] filteredSelection;
            if (RaiseSelectionChanging(uiObjects.Union(gameObjects).ToArray(), out filteredSelection))
            {
                if (multiselect)
                {
                    if (filteredSelection.Length != 0)
                    {
                        ...
                        else
                        {
                            ...
                            foreach(UnityObject obj in filteredSelection)
                            {
                                hs.Add(obj);
                            }

                            Selection.objects = hs.ToArray();
                        }
                    }
                }
                else
