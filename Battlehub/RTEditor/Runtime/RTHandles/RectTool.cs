...
namespace Battlehub.RTHandles
{
    public class RectTool : BaseHandle
    {
        ...
        [SerializeField]private int m_selectedEdgeIndex = -1;
        ...
        protected override Transform[] Targets_Internal
        {
            ...
            set
            {
                ...
                m_isInRectTransformMode = targets != null && targets.Length > 0 && targets.All(t => t is RectTransform);
            }
        }
        ...
        protected override void Awake()
        {
            ...
            {
                CanvasRenderer canvasRenderer1 = m_txtSize1.GetComponent<CanvasRenderer>();
                if (canvasRenderer1 != null)
                {
                    DestroyImmediate(canvasRenderer1);
                }

                CanvasRenderer canvasRenderer2 = m_txtSize2.GetComponent<CanvasRenderer>();
        ...
        protected override void OnEnable()
        {
            ...
            m_linesRenderer.sharedMaterial.SetFloat("_Scale", scale);
            m_pointsRenderer.sharedMaterial.SetFloat("_Scale", 4.5f * scale);
        }
        ...
        private Vector3 GetOffset(int selectedPointIndex, Vector3 pointOnPlane, out Vector3 refPoint)
        {
            ...
            Vector3 delta = m_referencePoints[m_selectedPointIndex] - m_beginDragPoint;
        ...
        private RuntimeHandleAxis GetAxis(out float dot)
        {
            ...
            Vector3 camForward = camera.orthographic ?
                ...
                (m_lines.transform.position - camera.transform.position).normalized;

            float dotZ = Vector3.Dot(m_lines.transform.forward, camForward);
            float dotY = Vector3.Dot(m_lines.transform.up, camForward);
            float dotX = Vector3.Dot(m_lines.transform.right, camForward);
        ...
        private void UpdateText()
        {
            ...
            if (m_txtSize1 != null)
            {
                if (IsAxisLocked(m_currentAxis))
                {
                    m_txtSize1.gameObject.SetActive(false);
                }
                else
                {
                    m_txtSize1.gameObject.SetActive(true);
            ...
            if (m_txtSize2 != null)
            {
                if (IsAxisLocked(m_currentAxis))
                {
                    m_txtSize2.gameObject.SetActive(false);
                }
                else
                {
                    m_txtSize2.gameObject.SetActive(true);
        ...
        private Vector3[] GetVertices(RuntimeHandleAxis axis, Bounds bounds)
        {
            ...
            if (axis == RuntimeHandleAxis.XY)
            {
                m_getVertRes[0] = c + new Vector3(e.x, e.y, 0);
                m_getVertRes[1] = c + new Vector3(-e.x, e.y, 0);
                m_getVertRes[2] = c + new Vector3(-e.x, -e.y, 0);
                m_getVertRes[3] = c + new Vector3(e.x, -e.y, 0);
                m_getVertRes[4] = c;
            }
            else if (axis == RuntimeHandleAxis.XZ)
            {
                m_getVertRes[0] = c + new Vector3(e.x, 0, e.z);
                m_getVertRes[1] = c + new Vector3(-e.x, 0, e.z);
                m_getVertRes[2] = c + new Vector3(-e.x, 0, -e.z);
                m_getVertRes[3] = c + new Vector3(e.x, 0, -e.z);
                m_getVertRes[4] = c;
            }
            else
            {
                m_getVertRes[0] = c + new Vector3(0, e.y, e.z);
                m_getVertRes[1] = c + new Vector3(0, -e.y, e.z);
                m_getVertRes[2] = c + new Vector3(0, -e.y, -e.z);
                m_getVertRes[3] = c + new Vector3(0, e.y, -e.z);
                m_getVertRes[4] = c;
            }
                ...
                if (pointPickResult.Index != m_selectedPointIndex)
                {
                    ...
                    if (m_selectedPointIndex >= 0)
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < 4; ++i)
                            {
                                colors[m_selectedPointIndex * 4 + i] = color;
                            }
                        }
                    }

                    m_selectedPointIndex = pointPickResult.Index== 4 ? -1 : pointPickResult.Index;
