...
namespace Battlehub.RTHandles
{
    public class PositionHandleModel : BaseHandleModel
    {
        ...
        {
            get { return m_arrowRadius; }
            set { m_arrowRadius = value; }
        }
        ...
        {
            get { return m_quadLength; }
            set { m_quadLength = value; }
        }
        ...
        private float _QuadLength
        {
            get
            {
                ...
                return Appearance.PositionHandleArrowOnly ? 0 : m_quadLength;
            }
        }
        [SerializeField]
        ...
        {
            float quadLength = Mathf.Abs(_QuadLength);
            ...
            else
            {
                for (int i = 0; i < m_colliders.Length; ++i)
                {
                    ...
                    if (m_colliders[i].Raycast(ray, out hit, Window.Camera.farClipPlane))
                    {
                        ...
                        if(hit.collider == m_xyCollider || hit.collider == m_xzCollider || hit.collider == m_yzCollider)
                        {
                            hit.distance *= 0.1f;
                        }
