...
namespace Battlehub.RTHandles
{
    ...
    public class RuntimeHandlesComponent : MonoBehaviour, IRuntimeHandlesComponent
    {
            ...
            if (handles == null)
            {
                if (s_instance == null)
                {
                    ...
                    if (s_instance == null)
                    {
                        ...
                        s_instance = runtimeHandles.AddComponent<RuntimeHandlesComponent>();
                    }
                }
                handles = s_instance;
            }
        ...
        public void DoScaleHandle(IRTECommandBuffer commandBuffer, Camera camera, RTHDrawingSettings settings, bool isUniform = false)
        {
            ...
            if (isUniform)
            {
                RuntimeHandleAxis axis = selectedAxis != RuntimeHandleAxis.None ? RuntimeHandleAxis.XYZ : RuntimeHandleAxis.None;
                DoAxes(commandBuffer, propertyBlocks, linesTransform, axis, xLocked, yLocked, zLocked, drawLocked);
            }
            else
            {
                DoAxes(commandBuffer, propertyBlocks, linesTransform, selectedAxis, xLocked, yLocked, zLocked, drawLocked);
            ...
            {
                DrawMesh(commandBuffer, drawLocked || !xLocked, xLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position + xOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !yLocked, yLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position + yOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !zLocked, zLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position + zOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !xyzLocked, xyzLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position, rotation, screenScale * 1.35f), m_shapesMaterialZTest);
            }
            else if (selectedAxis == RuntimeHandleAxis.X)
            {
                DrawMesh(commandBuffer, drawLocked || !xLocked, xLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position + xOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !yLocked, yLocked ? DisabledCube : CubeY, Matrix4x4.TRS(position + yOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !zLocked, zLocked ? DisabledCube : CubeZ, Matrix4x4.TRS(position + zOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !xyzLocked, xyzLocked ? DisabledCube : CubeUniform, Matrix4x4.TRS(position, rotation, screenScale * 1.35f), m_shapesMaterialZTest);
            }
            else if (selectedAxis == RuntimeHandleAxis.Y)
            {
                DrawMesh(commandBuffer, drawLocked || !xLocked, xLocked ? DisabledCube : CubeX, Matrix4x4.TRS(position + xOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !yLocked, yLocked ? DisabledCube : SelectionCube, Matrix4x4.TRS(position + yOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !zLocked, zLocked ? DisabledCube : CubeZ, Matrix4x4.TRS(position + zOffset, rotation, screenScale), m_shapesMaterialZTest);
                DrawMesh(commandBuffer, drawLocked || !xyzLocked, xyzLocked ? DisabledCube : CubeUniform, Matrix4x4.TRS(position, rotation, screenScale * 1.35f), m_shapesMaterialZTest);
            }
        ...
        public void DoSceneGizmo(IRTECommandBuffer commandBuffer, MaterialPropertyBlock[] propertyBlocks, Camera camera, Vector3 position, Quaternion rotation, Vector3 selection, float gizmoScale, Color textColor, float xAlpha = 1.0f, float yAlpha = 1.0f, float zAlpha = 1.0f)
        {
            ...
            if (selection != Vector3.zero)
            {
                ...
                {
                    commandBuffer.DrawMesh(SceneGizmoSelectedCube, Matrix4x4.TRS(position, rotation, screenScale * cubeScale), m_shapesMaterialZTestOffset, 0, -1);
                }
                else
                {
                    ...
                    {
                        commandBuffer.DrawMesh(SceneGizmoSelectedAxis, Matrix4x4.TRS(position, rotation * Quaternion.LookRotation(selection, Vector3.up), screenScale), m_shapesMaterialZTestOffset, 0, -1);
                    }
                }
            }

            m_shapesMaterialZTest.color = Color.white;
            commandBuffer.DrawMesh(SceneGizmoCube, Matrix4x4.TRS(position, rotation, screenScale * cubeScale), m_shapesMaterialZTest, 0, -1);
            if (xAlpha == 1.0f && yAlpha == 1.0f && zAlpha == 1.0f)
            {
                propertyBlocks[0].SetColor("_Color", new Color(1, 1, 1, 1));
                commandBuffer.DrawMesh(SceneGizmoXAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest3, 0, -1, propertyBlocks[0]);
                propertyBlocks[1].SetColor("_Color", new Color(1, 1, 1, 1));
                commandBuffer.DrawMesh(SceneGizmoYAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest4, 0, -1, propertyBlocks[1]);
                propertyBlocks[2].SetColor("_Color", new Color(1, 1, 1, 1));
                commandBuffer.DrawMesh(SceneGizmoZAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest2, 0, -1, propertyBlocks[2]);
            }
            else
            {
                if (xAlpha < 1)
                {
                    propertyBlocks[0].SetColor("_Color", new Color(1, 1, 1, yAlpha));
                    commandBuffer.DrawMesh(SceneGizmoYAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest3, 0, -1, propertyBlocks[0]);

                    propertyBlocks[1].SetColor("_Color", new Color(1, 1, 1, zAlpha));
                    commandBuffer.DrawMesh(SceneGizmoZAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest4, 0, -1, propertyBlocks[1]);

                    propertyBlocks[2].SetColor("_Color", new Color(1, 1, 1, xAlpha));
                    commandBuffer.DrawMesh(SceneGizmoXAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest2, 0, -1, propertyBlocks[2]);
                }
                else if (yAlpha < 1)
                {
                    propertyBlocks[0].SetColor("_Color", new Color(1, 1, 1, zAlpha));
                    commandBuffer.DrawMesh(SceneGizmoZAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest4, 0, -1, propertyBlocks[0]);

                    propertyBlocks[1].SetColor("_Color", new Color(1, 1, 1, xAlpha));
                    commandBuffer.DrawMesh(SceneGizmoXAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest2, 0, -1, propertyBlocks[1]);

                    propertyBlocks[2].SetColor("_Color", new Color(1, 1, 1, yAlpha));
                    commandBuffer.DrawMesh(SceneGizmoYAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest3, 0, -1, propertyBlocks[2]);
                }
                else
                {

                    propertyBlocks[0].SetColor("_Color", new Color(1, 1, 1, xAlpha));
                    commandBuffer.DrawMesh(SceneGizmoXAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest2, 0, -1, propertyBlocks[0]);

                    propertyBlocks[1].SetColor("_Color", new Color(1, 1, 1, yAlpha));
                    commandBuffer.DrawMesh(SceneGizmoYAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest3, 0, -1, propertyBlocks[1]);

                    propertyBlocks[2].SetColor("_Color", new Color(1, 1, 1, zAlpha));
                    commandBuffer.DrawMesh(SceneGizmoZAxis, Matrix4x4.TRS(position, rotation, screenScale), m_shapesMaterialZTest4, 0, -1, propertyBlocks[2]);
                }
            }
            ...
            propertyBlocks[3].SetColor("_Color", new Color(c.r, c.b, c.g, xAlpha));
            DragSceneGizmoAxis(commandBuffer, propertyBlocks[3], m_xMaterial, camera, position, rotation, Vector3.right, gizmoScale, billboardScale, billboardOffset, sScale);

            propertyBlocks[4].SetColor("_Color", new Color(c.r, c.b, c.g, yAlpha));
            DragSceneGizmoAxis(commandBuffer, propertyBlocks[4], m_yMaterial, camera, position, rotation, Vector3.up, gizmoScale, billboardScale, billboardOffset, sScale);

            propertyBlocks[5].SetColor("_Color", new Color(c.r, c.b, c.g, zAlpha));
            DragSceneGizmoAxis(commandBuffer, propertyBlocks[5], m_zMaterial, camera, position, rotation, Forward, gizmoScale, billboardScale, billboardOffset, sScale);
        }
