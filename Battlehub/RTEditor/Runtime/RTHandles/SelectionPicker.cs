...
namespace Battlehub.RTHandles
{
    ...
    public class SelectionPicker
    {
        ...
        {
            return UnityObjectExt.FindObjectsByType<Renderer>().Where(r => r.hideFlags == HideFlags.None && r.isVisible).ToArray();
        }
        ...
        public Renderer[] EndPick(Color32[] texPixels, Vector2Int texSize, Renderer[] renderers = null)
        {
            if (renderers == null)
            {
                renderers = UnityObjectExt.FindObjectsByType<Renderer>().Where(r => r.hideFlags == HideFlags.None && r.isVisible).ToArray(); 
            }
