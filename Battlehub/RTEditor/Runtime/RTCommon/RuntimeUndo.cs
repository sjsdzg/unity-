...
namespace Battlehub.RTCommon
{
    ...
    public class RuntimeUndo : IRuntimeUndo
    {
        ...
        public bool IsRecordingValues
        {
            get { return m_objToValue.Count > 0; }
        }
        ...
        public void GroupRecords(int count)
        {
            ...
            var recordGroups = new Stack<Record[]>();
            ...
            while (recordGroups.Count > 0)
            {
                Record[] records = recordGroups.Pop();
                for (int i = 0; i < records.Length; ++i)
                {
                    m_group.Add(records[i]);
                }
            }
        ...
        private void RecordTransform(bool applyOnRedo, Transform target, Transform parent = null, int siblingIndex = -1, Action<Transform> callback = null)
        {
            ...
            CreateRecord(target, newState, null,
                ...
                (record, oldReference, newReference) =>
                {
                    if (newReference == null)
                    {
                        // Added during RuntimeAssetDatabase integration (3/13/2024)
                        // There is currently no easy way to undo/redo changes written to the runtime asset database
                        // When an object is converted to part of a prefab instance and becomes "attached", the user should not be able to move it in the hierarchy,
                        // manually or using undo/redo.
                        // This is a workaround that removes the reference to this object (it is better to remove such an entry from the stack completely)
                        // Before applying changes to the asset database, you must call the following methods
                        // IRTE rte = IOC.Resolve<IRTE>();
                        // 1. rte.Undo.Erase(child instances, null);
                        // 2. rte.Undo.Erase(child instances, null);
                        // 3. rte.Undo.EraseFromSelection(new[] {children marked as destroyed });
                        // 4. Destroy all child elements marked as destroyed
                        // (Good place for this is corresponding methods or RuntimeAssetDataBaseModel)
        ...
        public Component AddComponent(ExposeToEditor obj, Type type)
        {
            ...
            Type definingType;
            DisallowMultipleComponent disallowMultipleComponent = Reflection.GetCustomAttribute<DisallowMultipleComponent>(type, out definingType);
            if (disallowMultipleComponent != null)
            {
                Component alreadyAdded = obj.GetComponent(definingType);
                if (alreadyAdded != null)
                {
                    Debug.LogFormat("Can't add {0} because a {1} is already added to the game object!", type.Name, alreadyAdded.GetType().Name);
                    return null;
                }
            }
            ...
            Record newRecord = CreateRecord(obj, type, null,
                ...
                record =>
                {
                    ...
                    OnAfterAddComponentUndo(component.gameObject, component.GetType(), gameObjectState);
                    return true;
                },
                record =>
                {
            ...
            return newRecord.OldState as Component;
        }
        ...
        public Component[] AddComponentWithRequirements(ExposeToEditor obj, Type type)
        {
            ...
            {
                if (requirement.m_Type0 != null && !obj.GetComponent(requirement.m_Type0))
                {
                    var req0 = AddComponent(obj, requirement.m_Type0);
                    componentsList.Add(req0);
                }
                if (requirement.m_Type1 != null && !obj.GetComponent(requirement.m_Type1))
                {
                    var req1 = AddComponent(obj, requirement.m_Type1);
                    componentsList.Add(req1);
                }
                if (requirement.m_Type2 != null && !obj.GetComponent(requirement.m_Type2))
                {
                    var req2 = AddComponent(obj, requirement.m_Type2);
                    componentsList.Add(req2);
                }
            }

            var component = AddComponent(obj, type);
        ...
        public void DestroyComponent(Component destroy, MemberInfo[] memberInfo)
        {
            ...
            if(memberInfo == null)
            {
                memberInfo = Reflection.GetSerializableFields(destroy.GetType(), false)
                    .Cast<MemberInfo>()
                    .Union(Reflection.GetSerializableProperties(destroy.GetType())).ToArray();
            }
        ...
        private static void DestroyComponent(Component component)
        {
            if (component != null)
            {
                ...
                {
                    exposeToEditor.DestroyComponent(component);
                }
                else
                {
    ...
    public class DisabledUndo : IRuntimeUndo
    {
        ...
        public bool IsRecordingValues { get { return false; } }
