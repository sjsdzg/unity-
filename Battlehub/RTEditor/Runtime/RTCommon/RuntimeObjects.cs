    ...
    public class RuntimeObjects : MonoBehaviour, IRuntimeObjects
    {
        ...
        public IEnumerable<ExposeToEditor> Get(bool rootsOnly, bool useCache)
        {
            if(rootsOnly)
            {
                if (m_editor.IsPlaying)
                {
                    ...
                    return m_playModeCache.Where(o => o != null && o.GetParent(true) == null);
                }
                else
                {
                    if (!useCache)
                    ...
                    return m_editModeCache.Where(o => o != null && o.GetParent(true) == null);
                }
            }
        ...
        private void OnPlaymodeStateChanging()
        {
            if (m_editor.IsPlaying)
            {
                ...
                {
                    return obj.GetParent() == null && obj.transform.parent == sceneRoot;
                }

                var sceneRoot = m_editor.SceneRoot != null ? m_editor.SceneRoot.transform : null;
                var editCache = m_editModeCache.Where(eo => eo != null).OrderBy(eo => eo.transform.GetSiblingIndex());
                foreach (ExposeToEditor editorObj in editCache)
                {
                    if (!IsRootSceneObject(editorObj, sceneRoot))
                    {
                        continue;
                    }
                ...
                var fakeRootPlayMode = Instantiate(fakeRoot);
                foreach (ExposeToEditor editorObj in editCache)
                {
                    if (!IsRootSceneObject(editorObj, fakeRoot.transform))
                    {
                        continue;
                    }
                ...
                {
                    var tr = fakeRoot.transform;
                    int childCount = tr.childCount;
                    for (int i = 0; i < childCount; ++i)
                    {
                        Transform child = tr.GetChild(0);
                        child.SetParent(sceneRoot, true);
                    }

                    tr = fakeRootPlayMode.transform;
        ...
        private bool CanAddMeshCollider(ExposeToEditor obj, bool isRigidBody)
        {
            ...
            Mesh skinnedMesh = obj.SkinnedMeshRenderer != null ? obj.SkinnedMeshRenderer.sharedMesh : null;
            ...
            else if (skinnedMesh != null)
            {
                ...
                {
                    if (skinnedMesh.GetTopology(i) != MeshTopology.Triangles)
                    {
                        return false;
                    }
                }
        ...
        private void TryToAddColliders(ExposeToEditor obj)
        {
            ...
            if (obj.Colliders == null || obj.Colliders.Length == 0)
            {
                ...
                if (obj.EffectiveBoundsType == BoundsType.Any)
                {
                    ...
                    {
                        if (CanAddMeshCollider(obj, isRigidBody))
                        {
                ...
                //#warning Create a new version of runtime objects that bakes mesh colliders on a background thread.. All at once.
                for (int i = 0; i < colliders.Count; ++i)
                {
                    colliders[i].hideFlags = HideFlags.DontSave;
                }
            }
        }
        ...
        private void OnDestroyed(ExposeToEditor obj)
        {
            ...
            {
                var objects = m_editor.Selection.objects.Where(o => o != obj.gameObject).ToArray();
                if (objects.Length > 0) 
                {
                    m_editor.Selection.Select(objects.First(), objects);
                }
                else
                {
        ...
        public void SendMessageTo(GameObject gameobject, string methodName, params object[] parameters)
        {
            ...
            {
                if (m == null) // in unity 2021.1, sometimes the component is null https://github.com/Battlehub0x/RTE_Docs/issues/75
                {
                    continue;
                }
        ...
        private void OnComponentDestroyed(ExposeToEditor obj, Component component)
        {
            ...
            {
                ComponentDestroyed(obj, component);
            }
        }
